...
namespace cAlgo
{
    ...
    public class LngSclp_MM : Robot
    {
        [Parameter("Initial Target Ticks", DefaultValue = 16)]
        public int initTargetTicks { get; set; }

        [Parameter("Symetric Half Zone Pips", DefaultValue = 300)]
        public int halfZoneTicks { get; set; }

        [Parameter("Symetric Targets Beyond Zone Pips", DefaultValue = 300)]
        ...
        private const string label = "LngSclp_MM";
...
foreach (var pos in allPositions)
        ...
        private void CheckForEntry()
        {
            ...
            long vol = (long)Math.Max((Math.Floor(Account.Equity / 2000.0) * 1000), 1000);
