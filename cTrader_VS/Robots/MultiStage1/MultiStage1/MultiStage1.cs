...
//MultiStage1 cBot will enter on signal from LRF_PSAR_Tren
//if profitable, exit on LRF_PSAR_Trend, else
//pass exit control to Traililng stop (can be constant pip, or ATR scaled stop)-passing exit to trailing stop will free 
//LRF_PSAR_Trend to enter new trades even if trailing stop has not closed the prev. position.
//if trailing stop is adverse, pass control to hedging exit for ping pong style management
...
namespace cAlgo
{
    ...
    public class L2SSS220150909PM : Robot
    {
        [Parameter("Source")]//is close default value??
        ...
        [Parameter("minAccumF", DefaultValue = 0.02)]
        public double minAccumF { get; set; }

        [Parameter("maxAccumF", DefaultValue = 0.2)]
        public double maxAccumF { get; set; }

        [Parameter("Trailing Pips", DefaultValue = 20)]
        public double trailPips { get; set; }
        ...
        string label = "MultiStage1-20151011";

        private LRF_PSAR_Trend lrfpsar;

        private Dictionary<int, double> positionHWMdic = new Dictionary<int, double>();
        ...
        private void UpdateHighWaterMarks()
        {
            ...
            {
                double hwmPips;
                if (!positionHWMdic.ContainsKey(pos.Id)) positionHWMdic.Add(pos.Id, pos.Pips);
                else if(positionHWMdic.TryGetValue(pos.Id, out hwmPips))
                {
                    positionHWMdic[pos.Id] = pos.Pips > hwmPips ? pos.Pips : hwmPips;
                }
            }
        }
        ...
        {//modified to exit on opposite of entry signal
            foreach (var pos in Positions.FindAll(label,Symbol))
            {
                if (positionHWMdic.ContainsKey(pos.Id) && pos.Pips <= positionHWMdic[pos.Id] - trailPips)
                {
                    ClosePosition(pos);
                }
                else Print("could not find position {0} in dictionary",pos.Id);
            }
            ////double d = lrfpsar.Signal.LastValue;
            ////long longV=0L, shortV=0L;
            ////GetLongShortVolumes(ref longV, ref shortV);
            ////if ((d>0 && shortV>longV) || (d<0 && longV>shortV))
            ////{
