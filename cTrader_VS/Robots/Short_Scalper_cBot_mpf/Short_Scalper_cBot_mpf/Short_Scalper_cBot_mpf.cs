//moved profit factor to multiply P/L to overcome instead of on lots returned
using System;
using System.Linq;
using cAlgo.API;
...
namespace cAlgo
{
    ...
    public class Short_Scalper_cBot_mpf : Robot
    {
        [Parameter("Initial Target Ticks", DefaultValue = 30)]
        public int initTargetTicks { get; set; }

        [Parameter("Symetric Half Zone Pips", DefaultValue = 220)]
        public int halfZoneTicks { get; set; }

        [Parameter("Symetric Targets Beyond Zone Pips", DefaultValue = 340)]
        ...
        private const string label = "Short_Scalper_cBot_mpf";
...
foreach (var pos in allPositions)
        ...
        private long GetVolumeToBrkEvn(double pl, int ticksToTarget)
        {
            ...
            vol = Math.Ceiling((-1 * pl / (ticksToTarget * Symbol.TickValue)) / 1000);
        ...
        private void CheckForEntry()
        {
            ...
            ExecuteMarketOrder(TradeType.Sell, Symbol, vol, label, null, null, null, "1st");
