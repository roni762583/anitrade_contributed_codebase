//new ver. of Super Simple Scalper: 
// - add limit to slippage, 
// - send alerts as control log for positions, 
// - indicator for trading in direction of trend
...
namespace cAlgo
{
    ...
    public class NewSSS : Robot
    {
        ...
        [Parameter("Max. Slippage Pips", DefaultValue = 0.5)]
        public double maxSlippagePips { get; set;}

        string label = "newSSS";
        string from = "8053141tradealerts@gmail.com";
        string to = "8053141tradealerts@gmail.com";
        ...
        private void SendNotificationEmail(Position pos, string txt)
        {
            string lbl = pos.Label;
            string typ = pos.TradeType.ToString();
            string vol = pos.Volume.ToString();
            string subject = string.Format("{0}, {1}, {2}, {3}",txt,lbl,typ,vol);
            string text = string.Format("{0}, {1}",pos.Id,pos.EntryTime);
        ...
        {
            long vol = GetVolume();
            var tr = ExecuteMarketOrder(TradeType.Buy, Symbol, vol, label, null, null, maxSlippagePips, "1st");
        ...
        {
            long vol = GetVolume();
            var tr = ExecuteMarketOrder(TradeType.Sell, Symbol, vol, label, null, null, maxSlippagePips, "1st");
            //SetTradeVars(tr);
        }
        ...
        private void OpenHedge(TradeType tt, long vol)
        {
            ...
            var tr = ExecuteMarketOrder(tt, Symbol, vol, label, null, null, maxSlippagePips, "hdg");
            if (!tr.IsSuccessful) SendNotificationEmail(tr.Position, "ExecuteMarketOrder FAILED");
        }
