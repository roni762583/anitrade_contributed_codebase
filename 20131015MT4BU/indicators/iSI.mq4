...
int start()  {
   ...
   for(i = limit; i >= 0; i--)  {
      ...
      if(R == 9999999.98765) R = MathAbs(   (H2-C1) - (0.50*(L2-C1)) + (0.25*(C1-O1))    ); //i.e. in the rare case all are equal, use first option
      if(R==0.0)   { //catch R=0 devide by zero error
         Print("R=0.0, is set to 1.0 at datetime: ", TimeToStr(Time[i],TIME_DATE|TIME_MINUTES), " at index = ", i);
         R=1.0;
      }
      //set value for K
