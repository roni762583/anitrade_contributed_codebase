...
// d = MathAbs(Low[i]-Low[i-m])                             this is called Direction
// m                               is  a          user defined constant called Efficiency Ratio Period 
// v = m*(Sum(MathAbs(Low[j]-Low[j-1]), from j=i to j=i-m)  this is called Volatility
...
int start()   {
   ...
   if(counted_bars == 0) {
      ...
      prevVMA = Low[limit];                              //initial value
   }
   ...
   for(i = limit; i >= 0; i--)   {
      d = MathAbs(Low[i]-Low[i+m]);
      sum = 0.0;
      
      for(int j = i+m; j>=i+1; j--)   {
         sum = sum + MathAbs(Low[j-1]-Low[j]);
      }
      ...
      //Print("a=",a,"  d=",d,"  v=",v,"  y=",y,"  Low[i]=",Low[i],"  prevVMA=", prevVMA);
      
      result  = a*y*Low[i] + (1-a*y)*prevVMA;
