//+------------------------------------------------------------------+
//|                                          Aharon_Bands_on_MomOverATRSD.mq4 |
//|                      
...
//---- name for DataWindow and indicator subwindow label
   string short_name;
   short_name="BB_on_MomOverATR*SD("+SDperiod+", "+BandsDeviations+", "+BandsPeriod+")";
...
// Set values
   for(i=0; i<limit; i++)  {
      D[i]= (Close[i]-Close[i+1])/MathMax(iATR(Symbol(),0,ATRperiod,i)/MathMax(iStdDev(Symbol(),0,SDperiod,0,MODE_SMA,PRICE_CLOSE,i), Point/1000.0), Point/1000.0);
   }
   for(i=0; i<limit; i++)  {
