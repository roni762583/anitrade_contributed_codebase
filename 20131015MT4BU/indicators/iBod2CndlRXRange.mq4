//+------------------------------------------------------------------+
//|                                  iBod2CndlRXRange.mq4                  |
//|                           Copyright 2012, anitani Software Corp. |
//|                                           http://www.anitani.com |
//+------------------------------------------------------------------+
//take ratio of candle-body to total-range as coefficient to avg. true range: [(close-open)/(high-low)]* ATR(14)
...
int start()  {
   int i;
   for(i=0; i<Bars-1; i++)  {
      sigBuf[i] = ((Close[i]-Open[i])/(High[i]-Low[i]))*iATR(NULL, 0, atrPeriod, i);
