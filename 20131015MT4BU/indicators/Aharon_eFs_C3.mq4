//+------------------------------------------------------------------+
//|                                             Aharon_eFs_C3.mq4.mq4 |
//|                                                           Aharon |
//|                                           http://www.anitani.net |
//+------------------------------------------------------------------+

#property copyright "Copyright © 2009, Aharon"
#property link      "http://www.anitani.net"

#property indicator_separate_window

#property indicator_buffers 4
#property indicator_color1 Blue
#property indicator_color2 Red
#property indicator_color3 Green
#property indicator_color4 Pink

extern int    Period1 =   4;
extern int    Taps    =   5;  //must be odd number
extern int    Window  =   4;

extern int    OTF     =   1;  //lower timeframe

// buffers
double s0[], s1[], s3[], s4[];

int p;
//+------------------------------------------------------------------+
//| Custom indicator initialization function                         |
//+------------------------------------------------------------------+
int init()  {
   
   SetLevelValue(0,0.0);
   
   IndicatorBuffers(4);
   
   p = Period();
   
   string short_name;
   short_name = "eFs_C3(" + Period1 + ", " + Taps + ", " + Window + ", " + OTF + ")";
   IndicatorShortName(short_name);
   
   SetIndexLabel(0, StringConcatenate("Slp.Chrt ", p, " TF (B)"));
   SetIndexStyle(0,  DRAW_HISTOGRAM);
   SetIndexBuffer(0, s0);
   
   SetIndexLabel(1, StringConcatenate("Slp. OTF ", OTF, " TF (R)"));
   SetIndexStyle(1,  DRAW_HISTOGRAM);
   SetIndexBuffer(1, s1);
   
   SetIndexLabel(2, "Newly Agree (G)");
   SetIndexStyle(2,  DRAW_LINE);
   SetIndexBuffer(2, s3);
   
   SetIndexLabel(3, "Continuation (O)");
   SetIndexStyle(3,  DRAW_LINE);
   SetIndexBuffer(3, s4);
   
   return(0);
}


//+------------------------------------------------------------------+
//| Custom indicator iteration function                              |
//+------------------------------------------------------------------+
int start()  {
   int limit, i, shftI;
   int counted_bars=IndicatorCounted();
   if(counted_bars<0) return(-1);
   if(counted_bars>0) counted_bars--;
   limit=Bars-counted_bars;
   
   for(i=0; i<limit; i++)  {
         
      // in OTF, what is shift of bar that opens at the open time of bar i in HTF...
      s0[i] = iCustom(NULL, p, "Aharon_eFslope_C",  Period1, Taps, Window, 1, i); //chart timeframe slope
      shftI = iBarShift(NULL, OTF, iTime(NULL, p, i), false);                      //shift in OTF of bar with open time corresponding to open of chart bar
      s1[i] = iCustom(NULL, OTF, "Aharon_eFslope_C",  Period1, Taps, Window, 1, shftI);  //
   }              
   
   for(i=0; i<limit; i++)  {
      s3[i] = 0.0;
      if(s0[i]>0 && s1[i]>0 && s0[i+1]<0 && s1[i+1]<0) s3[i] = 1.0;  // agree up
      if(s0[i]<0 && s1[i]<0 && s0[i+1]>0 && s1[i+1]>0) s3[i] = -1.0;  // agree down
   }
   
   
   for(i=0; i<limit; i++)  {  // need to combine two for loops of s4 into one to prevent false trigering between loops
      //s4[i] = s3[i+1];
      if(s3[i+1] ==  1.0 && s0[i]>0 && s1[i]>0 ) s4[i] = 1.0;  // continuation
      if(s3[i+1] == -1.0 && s0[i]<0 && s1[i]<0 ) s4[i] = 1.0;  // continuation
   }
   for(i=0; i<limit; i++)  {//have to make a second run to catch the possibility of a continuation of s4[i+1]
      if(s4[i+1] ==  1.0 && s0[i]>0 && s1[i]>0 ) s4[i] = 1.0;  // continuation
      if(s4[i+1] == -1.0 && s0[i]<0 && s1[i]<0 ) s4[i] = 1.0;  // continuation
   }
   return(0);
}
//+------------------------------------------------------------------+


//+------------------------------------------------------------------+
//| Custom indicator deinitialization function                       |
//+------------------------------------------------------------------+
int deinit()  {
   return(0);
}