...
//+------------------------------------------------------------------+
//this is like ...ASI3, fix H/L lines to reach last bar (0)
...
int init()  {
   IndicatorBuffers(3);
   string short_name;
   short_name = "A_i_WWJ_ASI3_6(" + BarsBack + ")";
...
int start()  {
   ...
   for(i=BarsBack; i>=0; i--) {
      ...
      if(!nlv) {
         l[i+1]=l[i+2]; //if no new low value asigned, then reassign prev. value to new bar 
         l[i]  = l[i+2];  //this was added to assign l[0] without consideration
      }
      if(!nhv) {
         h[i+1]=h[i+2]; //if no new high value asigned, then reassign prev. value to new bar 
         h[i]  = h[i+2];  //this was added to assign h[0] without consideration
      }
   }
