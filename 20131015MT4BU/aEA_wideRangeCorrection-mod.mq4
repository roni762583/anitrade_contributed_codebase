...
/*
  add training stop - under framework.mq4
...
extern double     bandsDev          = 3.0; //in prev. ver was 2.0
extern double     tpPoints          = 10;  //set TP in points rather than fraction of signal bar xFracOfBarTP      = 2.1;//0.61; // 71%
...
extern int        minimumPipTarget  = 0;//this will be multiplied by point, so on 5-decimal broker, this will be 10 4-decimal pips, ~=2*spread
...
int start()  {
  ...
  if(latch)   {
     ...
     double ShortTP = NormalizeDouble(Ask-tpPoints*Point,Digits);//NormalizeDouble(High[bshft] - xFracOfBarTP*(High[0]-Low[bshft]), Digits );
     double LongSL  = NormalizeDouble(Low[bshft]  - SLptsFromBarExtrm*Point, Digits); 
     double LongTP  = NormalizeDouble(Bid+tpPoints*Point,Digits);//NormalizeDouble(Low[bshft]  + xFracOfBarTP*(High[bshft]-Low[0]), Digits );
