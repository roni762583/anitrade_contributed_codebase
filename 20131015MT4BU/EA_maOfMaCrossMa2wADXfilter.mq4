//+------------------------------------------------------------------+
//|                                   EA_maOfMaCrossMa2wADXfilter.mq4 |
...
extern int      SL = 250,
                ...
                ma1price  = 1, //0-close, 1-open, 5-typical (H+L+C)/3 iMA
                ma2period = 5,
                ma2method = 2; 
...
int start()  {
   ...
   s1 = iCustom(NULL, 0, "maOfMaCrossMa", 50, Period(), ma1period, 0, ma1method, ma1price, ma2period, 0, ma2method, 0, 0); 
   
   s2 = iCustom(NULL, 0, "A_i_WWJ_DELTA_ADX", 500, 200, 1, 0.6, 0, 0); // adx delta
   s3 = iCustom(NULL, 0, "A_i_WWJ_DELTA_ADX", 500, 200, 1, 0.6, 1, 0); // ubb(setting)
   ...
   if(s1==1.0 && isFlat() && s4 == 1.0)  OrderSend(Symbol(), OP_BUY,  1, Ask, 3, buyStopLoss(), buyTakeProfit(), "test", 6718, 0, Green);
   if(s1==-1.0 && isFlat() && s4 == 1.0) OrderSend(Symbol(), OP_SELL, 1, Bid, 3, sellStopLoss(), sellTakeProfit(), "test", 6718, 0, Red);
