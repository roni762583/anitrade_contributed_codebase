...
bool   nb = false;                                  // to hold answer of newBar() for itteraton
...
// Trading criteria
   ...
   // signal0 is for range exceeding upper band
   s0 = iCustom(Symb, Period(), "Aharon_Bands_on_Range", 0, 0);//range line
   s1 = iCustom(Symb, Period(), "Aharon_Bands_on_Range", 1, 0);//upper band line
   if(s0>s1 && Open[0]<Close[0]) signal0 = 1; //range penetrated upper band and last bar is up
   if(s0>s1 && Open[0]>Close[0]) signal0 = -1; //range penetrated upper band and last bar is up
   
   if(nb && signal0 == 1 && isFlat())     {     // if new bar & last sig. up, and position is flat
      Opn_B=true;                                          // flag to open buy position
      Print("Opn_B flagged");
   }
   
   if (nb && signal0 == -1 && isFlat())    {     // if sig. just turned down, and position is flat
      Opn_S=true;                                          // flag to open short position
       Print("Opn_S flagged");
   }
   
   if ( nb && signal1==0 && !isFlat())   {           // if newBar, and signal on prev. bar is OFF, and is NOT flat, then, flatten position
      if(signal2==1) Cls_S=true; 
      if(signal2==-1)Cls_B=true;                                          // Criterion for closing orders 
   }
...
// Opening orders
   while(true)   {
     ...
     if(Opn_S==true)   {                                   // No opened orders
         ...
         Ticket = OrderSend(Symb, OP_SELL, Lts, Bid, 2, 0, 0, NULL, magic, 0, Red);   //Opening Sell
         if(Ticket > 0)  {                                 // Changed to > , Success :)
