...
int init()  {
   IndicatorBuffers(3);
   string short_name;
   short_name = "A_i_WWJ_SI-Angle-BB(" + BarsBack + ", " + bandsDeviation + ", " + bandsPeriod + ", " + bandsFactor + ")";
   IndicatorShortName(short_name);

   SetIndexLabel(0, "si-angle");
...
int start()  {
  ...
  //Add start date for for loop
   for(i=Bars-BarsBack; i>=0; i--) { 
      ...
      if(R != 0)   {
         si[i] = MathArctan((MathRound((50*( N / R ) * (K/L))/Point))/Period())*57.2957795; //angle in degrees of delta si / delta time in minutes
      }   
         else si[i] = si[i+1];
   }
