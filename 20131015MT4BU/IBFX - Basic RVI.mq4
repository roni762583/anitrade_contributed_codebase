...
{    
     ...
     if( UseCompletedBars )
     {
      ...
      {
         //---- Indicator 1 Values
         double Indicator1CurrentValue  = iRVI(NULL,0, Periods,MODE_MAIN,0+RealTime);
         double Indicator1PreviousValue = iRVI(NULL,0, Periods,MODE_MAIN,1+RealTime);
         //---- Indicator 2 Values
         double Indicator2CurrentValue  = iRVI(NULL,0, Periods,MODE_SIGNAL,0+RealTime);
         double Indicator2PreviousValue = iRVI(NULL,0, Periods,MODE_SIGNAL,1+RealTime);
...
int EnterShrt( string FinalSymbol, double FinalLots, string EA_Comment )
{
   ...
   while( !OrderLoop )
   {
      ...
      Ticket = OrderSend( FinalSymbol, OP_SELL, FinalLots, SymBid, 0, 0, 0, EA_Comment, MagicNumber, 0, CLR_NONE ); 
