//+------------------------------------------------------------------+
//|                                a_EA_consecutiveCorrection2.3.mq4 |
...
// ver 2.3 adds atr as tp in case tp is set to zero (adaptive to bar size)
...
int start()   {
   ...
   if(cntr == consecBars && flat())   {
      SL=Ask+sl*Point;
      if(TP == 0) tp=NormalizeDouble(Bid-iATR(NULL,0, 14, 1), Digits);//11272011
   ...
   if(cntr == consecBars && flat())   {//min. consec dn bars and flat position...
      SL=Bid-sl*Point;
      if(TP == 0) tp=NormalizeDouble(Ask+iATR(NULL,0, 14, 1), Digits);//11272011
