...
extern double ThldDecimal = 1.1;                           // for signal1
...
extern int    TF1        = PERIOD_M5;                      // Time Frame for signal1 - this overrules chart Time Frame!
...
// Trading criteria
   ...
   signal1 = iCustom(NULL, TF1, "Aharon_FMSlopeWBBOrdered", BandsDeviations, BandsPeriod, ThldDecimal, 0, 0);
   ...
   if (signal1 == 1.0 && Total==0)     {                   // if flat and signal up;                           
      Opn_B=true;                                          // Criterion for opening Buy
   }
   
   if (signal1 == -1.0 && Total==0)     {                  // if flat and signal down;                           
      Opn_S=true;                                          // Criterion for opening Sell
   }
     
   if (signal1 == 0.0 && Total > 0  && Total < 2)   {      // if not flat, and signal1  = 0, then flatten positions (Total < 2 is double percaution for muti-orders)
