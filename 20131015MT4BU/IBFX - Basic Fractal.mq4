...
{    
     ...
     if( UseCompletedBars )
     {
      ...
      {
         double F1 = iHigh( Sym, TimeFrame, iHighest( Sym, TimeFrame, MODE_HIGH, 5, Shift+1 )) ;
         double F2 =  iLow( Sym, TimeFrame,  iLowest( Sym, TimeFrame, MODE_LOW, 5, Shift+1 ) );
         
              if( High[0+RealTime]  > F1 && F1 != 0.0  )  { EnterLong(Sym, Lots, ""); }
         else if(  Low[0+RealTime]  < F2 && F2 != 0.0  )  { EnterShrt(Sym, Lots, ""); }
      }
      //----
