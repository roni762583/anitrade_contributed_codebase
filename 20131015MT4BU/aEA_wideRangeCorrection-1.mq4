//                                      aEA_wideRangeCorrection-1.mq4  |
//+------------------------------------------------------------------+
//-1 ver. add use of money management and lot size
...
extern double     xFracOfBarTP      = 0.75;//0.61; // 71%
extern double     bandsDeviation    = 2.0;
extern int        minRangeToTrigger = 0;
extern int        SLptsFromBarExtrm = 9000; //
...
extern bool       useMoneyMgmt      = false;  //flag to switch money management feature on/off
extern double     moneyMgmtBasisPts = 1;
extern double     lotSetVolume      = 1.0;//0.01;   //fixed volume to use when money management feature is off
//extern double lot = 1;
       ...
       // (latchBarTime != Time[0] && Close[1]>Open[1])      )  {     //is new bar following up bar ... reset flags & execute order 
        ...
        if(isFlat() && 
           ...
           xFracOfBarTP*(High[0]-Low[bshft])>=minimumPipTarget*Point) //if TP meets minimum criteria
              OpenShort(LotsVolume(), ShortSL, ShortTP);                       //three param. are: lots, SL, & TP
     }//close if((Ask+tra
      ...
      //  (latchBarTime != Time[0] && Close[1]<Open[1])      )   {    //is new bar following down bar ... reset flags & execute order 
        ...
        if(isFlat() && 
           ...
           xFracOfBarTP*(High[bshft]-Low[0])>=minimumPipTarget*Point) //if TP meets minimum criteria
              OpenLong(LotsVolume(), LongSL, LongTP);              //three param. are: lots, SL, & TP
     }//close if((Bid-trai...
  }//close if(latch)...
...
double LotsVolume()   {
   if(useMoneyMgmt)   {
      return(NormalizeDouble(AccountFreeMargin()*moneyMgmtBasisPts/10000, Digits));
   }
   else return(NormalizeDouble(lotSetVolume,Digits));
}
