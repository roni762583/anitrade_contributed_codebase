...
extern double TakeProfit = 10;                             // for an opened order

extern int    Period1    = 4;
extern int    Taps       = 21;
extern int    Window     = 4;

extern int    LTF        = 15;                             // long timeframe daily chart = 1440 min.

extern int    maxTime    = 3600;                           // kill order after No. of Sec. - freshness criteria
extern bool   useTimeOpt = true;
...
// Trading criteria
   ...
   signal0   = iCustom(NULL, 0, "Aharon_eFslope_HL",  Period1, Taps, Window, 1, 0);
   ...
   if (signal0>0 && signal1<0 && signalLTF>0)     {                                     
      Opn_B=true;  
     // Cls_S=true;                                        // Criterion for opening Buy
   }
   ...
   // close order criteria
   if (Tip==0)   {  //if BUY type
      //Close by order freshness (time )
      if(useTimeOpt) maxTime = TimeOpt(maxTime);
      if(TimeCurrent()-OrderOpenTime()> maxTime) OrderClose(OrderTicket(),OrderLots(),Bid,3,White);
   }

   if (Tip==1)   {  //if SELL type
      //Close by order freshness (time )
      if(useTimeOpt) maxTime = TimeOpt(maxTime);
      if(TimeCurrent()-OrderOpenTime()> maxTime) OrderClose(OrderTicket(),OrderLots(),Bid,3,White);
   }
...
//--------------------------------------------------------------- 8 --
   ...
   while(true)   {                                         // Orders closing loop
      ...
      if ( Opn_B==true)    {                    // No new orders +Total==0 &&
         ...
         Ticket = OrderSend(Symb, OP_BUY, Lts, Ask, 2, getSL("b"), getTP("b"), NULL, 0, 0, Blue);   //Opening Buy
      ...
      if (  Opn_S==true)   {                     // No opened orders +Total==0
         ...
         Ticket = OrderSend(Symb, OP_SELL, Lts, Bid, 2, getSL("s"), getTP("s"), NULL, 0, 0, Red);   //Opening Sell
