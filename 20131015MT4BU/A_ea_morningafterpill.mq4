...
extern int trendTP    = 20; //in pips
extern int trendSL    = 60; //in pips
extern int    swingTP  = 10;   //in pips
extern int    swingSL  = 50;  //in pips
...
void CheckForOpen()   {
   ...
   if(mode==1)  {                                                                            //if in trend mode ...
      
      //buy criteria
      if(Ask > (lastprice + increment*Point) && BuyOK)   {                                   //if price moved up and Buys are OK'd
   ...
   if(mode==-1)  {                                                                            //if in swing mode ...
      //buy criteria
      if(Bid < (lastprice - increment*Point) && BuyOK)   {                                   //if price moved down and Buys are OK'd
         res=OrderSend(Symbol(),OP_BUY,Lots,Ask,3, 0, 0,"",MAGICMA,0,Blue);  //then Buy
         if(res>0)  {
            OrderSelect(res, SELECT_BY_TICKET);
            lastprice = OrderOpenPrice();      //if went through, set lastprice to order price 
      ...
      //sell criteria
      if(Ask > (lastprice + increment*Point) && SellOK)  {                                  //if price moved up and and Sells are OK'd
        res=OrderSend(Symbol(),OP_SELL,Lots,Bid,3, 0,0,"",MAGICMA, 0,Red);  //then Sell
        if(res>0)  {
           OrderSelect(res, SELECT_BY_TICKET);
           lastprice = OrderOpenPrice();      //if went through, set lastprice to order price 
...
void CheckForClose()  {
   ...
   for(int i=0;i<OrdersTotal();i++)   {
      ...
      //set SL and TP for different modes
      
      if(mode==1)  {   //tren mode
         TP = trendTP*Point;
         SL = trendSL*Point;
      }
      if(mode==-1)  {   //swing mode 
         TP = swingTP*Point;
         SL = swingSL*Point;
      }         
...
void interpretPL()   {
//   if(mode==1 && spl<0 ) SellOK = false; //if trend mode, ans sells are lossing, don't OK more sells
//   if(mode==1 && bpl<0 ) BuyOK  = false; //if trend mode, ans buys  are lossing, don't OK more buys
   return;
}
