...
//                       Copyright © 2013
//last touch: 09/10/2013
//moved comments for development to bottom
            ...
            if(primed)   {//this block will only run after prog. has been running for at least length of time of timewindow
               ...
               if(newBid)  {
                  ...
                  if(direction != prevDirection)    {
                     if(direction==  1)   {
                        if(pNonZeroDirect== -1) Print("TURNED UP, [BUY] CHANGED TO SELL BID = ", Bid);
                        pNonZeroDirect = direction;
                     }
                     if(direction== -1)   {
                        if(pNonZeroDirect==  1) Print("TURNED DN, [SEL]CHANGED TO BUY ASK = ", Ask);
                        pNonZeroDirect = direction;
                     }
               ...
               //displays on chart
               Comment("Last Touch now" + "\n" +
                       "Moving Time Window [sec.]: " + DoubleToStr(tmWindow/1000.0,1) + "\n" + 
