...
extern double    n                 = 21;                   //period of MA on bidvelsumavg
         ...
         if(useTimeWindow)  {
            ...
            if(primed)   {//this block will only run after prog. has been running for at least length of time of timewindow
               ...
               if(newBid)  {
                  ...
                  bidvelsumavg = ((n-1)/n)*prevBidVelSumAvg+(bidvelsumavg/n); //upadte 04/04/'13
                  ...
                  if(bidBufferCounter<VIDYA_N+SMAoVIDYAperiod2) s = "Bids buffer not fully primed yet...";
                  if(bidBufferCounter>VIDYA_N+SMAoVIDYAperiod2)   {         //don't display unless calculation is primed with min. Bids
                     ...
                     ObjectSetText(vdname, CharToStr(158), 10, "Wingdings", Blue);
                     ...
                     ObjectSetText(smavdname, CharToStr(158), 10, "Wingdings", Yellow);
                     ...
                     ObjectSetText(smamodname, CharToStr(158), 10, "Wingdings", Red);
                  ...
                  if(/*VMA[0]>SMAoVIDYAbids && */SMAoVIDYAbids>SMAoVIDYAbids2) direction =  1;  //VMA[0], VIDYA, & VIDYA2 aligned up
                  if(/*VMA[0]<SMAoVIDYAbids && */SMAoVIDYAbids<SMAoVIDYAbids2) direction = -1;  //VMA[0], VIDYA, & VIDYA2 aligned down
                  ...
                  if(direction == 0)   {
                     ...
                     ObjectSet("directionalArrow", OBJPROP_PRICE1, High[0]+Point*10);
                  ...
                  if(direction != prevDirection && drawDirectionMark) { //if changed direction, mark on chart, and sound alerts
               ...
               Comment("Moving Time Window [sec.]: " + DoubleToStr(tmWindow/1000.0,1) + "\n" + 
                       "T.W. Bid Velocity [pps]: " + bidvel + "\n" +
                       "High Velocity Alert Threshold settings: " + posHighVelThshld + "  <--->  " + negHighVelThshld + "\n" +
                       "\n" + 
                       "Bid Directional Count: " + dct + "\n" +
                       "Bid Velocity Sum - its SMA(" + DoubleToStr(n,0) +") = " + (bidvelsum-bidvelsumavg) + "\n" +
                       "\n" +
                       s + "VIDYA on Bids(" + "m=" + VIDYA_m + ", N=" + VIDYA_N + ") = " + VMA[0] + "\n" +
                       "SMA(VIDYA(bids)) Delta("+DoubleToStr(SMAoVIDYAperiod,0)+","+DoubleToStr(SMAoVIDYAperiod2,0)+")= " + (SMAoVIDYAbids-SMAoVIDYAbids2)  );
