...
//////////////////////     Preliminary processing    /////////////////////
   ...
   if(TimeToStr(Time[0], TIME_MINUTES) == ResetTime) TradeExecuted = false;     //Reset TradeExecuted flag at midnight
   //Print("TimeToStr(Time[0], TIME_MINUTES) , ResetTime =", TimeToStr(Time[0], TIME_MINUTES) ,",", ResetTime);
...
//--------------------------------------------------------------
   ...
   if(SetSupportResistance() && IsTradingTime() && !TradeExecuted)   {  //
     if(Ask > Res) Opn_B = true;
        else if(Bid < Sup) Opn_S = true;
   }
...
bool SetSupportResistance()   {
   if( TimeToStr(Time[0],TIME_MINUTES) == SR_Levels_To_Time)   {
